 ...
 */
export function bidWithFlow(owner: string, buyer: string, assetId: string, price: string, amount: number): (CadenceEngine) => Promise<void> {
    if (!/^-?\d+(\.\d+)$/.test(price))
    ...
    return (engine: CadenceEngine) => {
        ...
        const code = engine.getCode("transactions/market/bid-with-flow");
